...
class Player(pygame.sprite.Sprite):
    ...
    def input(self):
        if not self.attacking:
            ...
            if keys[pygame.K_LCTRL]:
                ...
                strength = list(magic_data.values())[self.magic_index][
    ...
    def animate(self):
        annimation = self.animations[self.status]

        # loop over the frame index
        self.frame_index += self.animation_speed
        if self.frame_index >= len(annimation):
            self.frame_index = 0

        # set image
        self.image = annimation[int(self.frame_index)]
